...
configDefault = {
	...
	loadCheerImgs			: false,						// 비트 후원채팅을 이미지로 표시할지
  ...
  cheersMsg         : "",
	...
	loadClipPreview		: false,						// 클립 미리보기를 이미지로 표시할지
	...
	muteUser					: ["Nightbot", "Ssakdook", "twipkr"],			/* html에 표시하지 않을 유저 nickname
...
addChatMessage = function(nick, message, data) {
    ...
    chatOuterBox.style.left = (Math.random() * 92 + 4) * (100 / 96) + '%'
    chatOuterBox.style.top = Math.random() * 100 + '%'
    setTimeout(function() {
        chatOuterBox.classList.add('appear')
    }, 50)
    setTimeout(function() {
        chatOuterBox.classList.remove('appear')
        setTimeout(function() {
            document.getElementById("chat_wrapper").removeChild(chatOuterBox)
        }, 1000)
    }, 8000)
